...
const payload_settings = [
	...
	{
		"moduleid": "scb:logging:logger1",
		"mappings": {
			"Logintervall": {
				id: "scb.logging.logger1.Logintervall",
				type: 'int'
			}
		}
	},
	{
		"moduleid": "scb:logging:logger2",
		"mappings": {
			"Logintervall": {
				id: "scb.logging.logger2.Logintervall",
				type: 'int'
			}
		}
	},
	{
		"moduleid": "scb:rse/Inverter",
		"mappings": {
			"PowerCtrlBroadcast:Mode": {
				id: "scb.rse.InverterPowerCtrlBroadcastMode",
				type: 'int'
			}
		}
	},
	...
	apiCall('POST', 'processdata', payload, function(body, code, headers) {
		...
		apiCall('POST', 'settings', payload_2, function(body, code, headers) {
			if(code === 200) {
				...
				adapter.log.warn('Requesting settings - ' + JSON.stringify(payload_2) + ') failed with code ' + code + ': ' + body);
			}
...
function setObjects() {
	...
	/** settings logging */
	adapter.setObjectNotExists('scb.logging.logger1.Logintervall', {
		type: 'state',
		common: {
			name: 'Logintervall',
			type: 'number',
	...
	adapter.setObjectNotExists('scb.logging.logger2.Logintervall', {
		type: 'state',
		common: {
			name: 'Logintervall',
			type: 'number',
	...
	/** settings rse */
	adapter.setObjectNotExists('scb.rse.InverterPowerCtrlBroadcastMode', {
		type: 'state',
		common: {
			name: 'InverterPowerCtrlBroadcastMode',
			type: 'number',
			role: 'value',
			read: true,
			write: false
	...
	adapter.setObjectNotExists('scb.system.State', {
		type: 'state',
		common: {
			name: 'System State',
			type: 'number',
